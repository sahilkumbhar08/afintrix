                  ...
                  <div class="flex gap-5 max-md:flex-col max-md:gap-0">
                    <article class="flex flex-col w-[33%] max-md:ml-0 max-md:w-full">
                      <div class="flex flex-col grow text-base leading-6 text-slate-500 max-md:mt-10">
                        ...
                        />
                        <p class="mt-2">
                          <strong class="font-semibold text-slate-800">Remove Afintrix branding.</strong>
                ...
                >
                  <div class="flex w-full">
                    <p class="text-blue-500 capitalize font-medium flex-grow">
                      Afintrix - {{ currentPlan == 'default' ? 'Pro' : 'Team' }} plan
                    </p>
                    <UBadge
...
const props = defineProps({
  ...
  modal_description: {
    type: String,
    default: 'Unlock all features and get the most out of Afintrix.'
  },
  plan: {
    type: String,
    default: 'default'
...
watch(broadcastData, () => {
  ...
  if (broadcastData.value.type === 'success') {
    ...
    if (user.value.has_enterprise_subscription) {
      useAlert().success(
        'Awesome! Your subscription to Afintrix is now confirmed! You now have access to all Team '
        + 'features. No need to invite your teammates, just ask them to create a Afintrix account and to connect the same Notion workspace. Feel free to contact us if you have any question 🙌'
      )
    } else {
      useAlert().success(
        'Awesome! Your subscription to Afintrix is now confirmed! You now have access to all Pro '
